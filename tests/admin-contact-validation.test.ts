...
describe("admin contact validation", () => {
  ...
  it("accepts Indigenous Group in edits and role filters", () => { const update = form(); update.set("roles", "Indigenous Group"); expect(validateAdminContactUpdate(update)).toMatchObject({ success: true, data: { roles: ["Indigenous Group"] } }); expect(parseAdminListParams({ role: "Indigenous Group" })).toMatchObject({ role: "Indigenous Group" }); });
