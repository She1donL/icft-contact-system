...
function FieldLabel({ children, required }: { children: string; required: boolean }) {
  return <span className={styles.labelRow}><span>{children}</span><Requirement required={required} /></span>;
}
...
export function ContactForm({ privacy, submitLabel }: { privacy?: string; submitLabel?: string }) {
  ...
  return <form action={formAction} className={styles.form} noValidate>
    ...
    <fieldset aria-describedby="roles-error"><legend>Role(s) <Requirement required /></legend>{roleOptions.map((role) => <label className={styles.choice} key={role}><input type="checkbox" name="roles" value={role} onChange={(event) => role === "Other" && setHasOtherRole(event.target.checked)} />{role}</label>)}</fieldset><FieldError id="roles-error" message={errors.roles} />
